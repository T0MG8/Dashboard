...
# Aantal Afspraken per Maand per Jaar plot
...
# Toegewezen facturen per maand plot
fig1 = px.line(
        factuur, 
        x="maand", 
        y="totaalbedrag", 
        ...
        labels={"maand": "Maand", "totaalbedrag": "Totaalbedrag (€)", "jaar": "Jaar"},
    )
...
df_groep['maand'] = pd.Categorical(df_groep['maand'], categories=range(1,13), ordered=True)
...
if pagina_keuze == "Aantal Afspraken per Maand":
    ...
    df_groep_filtered = df_groep[df_groep['maand'].isin(range(maand_start + 1, maand_einde + 2))]
    factuur_grouped_filtered = factuur_grouped[factuur_grouped["maand_naam"].isin(geselecteerde_maanden_namen)]

    # Update de figuren met gefilterde gegevens
    fig = px.line(df_groep_filtered, x='maand', y='aantal', color='jaar', markers=True,
                  title='Aantal Afspraken per Maand per Jaar')
    fig.update_traces(hovertemplate='<br>Aantal: %{y}<br>')

    fig1 = px.line(factuur_grouped_filtered, x="maand_naam", y="totaalbedrag", color="jaar", markers=True, 
                   title="Toegewezen facturen per maand (2020-2024)", labels={"maand_naam": "Maand", "totaalbedrag": "Totaalbedrag (€)", "jaar": "Jaar"})
    fig1.update_traces(hovertemplate='<br>Aantal: %{y}<br>')

    # Weergave van de grafieken
    st.plotly_chart(fig)
